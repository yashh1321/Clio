...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /login (explicit navigation to http://localhost:3000/login) as the next immediate action.
        await page.goto("http://localhost:3000/login", wait_until="commit", timeout=10000)
        
        # -> Input 'student' into the username field (index 165) then fill password and click Login (index 167).
        ...
        # -> Enter 'Untargeted Submission' into the title field, click 'Submit Essay' without selecting a teacher, then look for the text 'Select' to verify validation is shown.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/header/div[1]/div[2]/input[1]').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('Untargeted Submission')
