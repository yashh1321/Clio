...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Let's Go' button to proceed to the login flow.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div/main/div[2]/a/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Type the student username into the username field, then type the password, then click Login (inputs: username=e2e_student, password=student123).
        frame = context.pages[-1]
        # Input text
        ...
        # -> Click the 'Register' link (index 383) to navigate to the account registration page because login failed; then create a new student account if required.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div/main/p/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Fill the registration form to create a student account (username=e2e_student, full name='E2E Student', email=e2e_student@example.edu, class='CSE-2024', course='Computer Science', password='student123') and click 'Create Account'.
        frame = context.pages[-1]
        # Input text
        ...
        await page.wait_for_timeout(3000); await elem.fill('e2e_student@example.edu')
        
        # -> Fill Class, Course, Password, Confirm Password fields and click 'Create Account' (index 642).
        frame = context.pages[-1]
        # Input text
        ...
        # -> Fill the Confirm Password field (index 641) with 'student123' and click the Create Account button (index 642) to create the student account.
        frame = context.pages[-1]
        # Input text
        ...
        # -> Click the 'Sign In' button to return to the login page so a different login approach (e.g., teacher_test) can be attempted.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div[2]/div/div/form/p/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Fill username with 'teacher_test' (index 908), fill password with 'teacher123' (index 912), then click the Login button (index 913) to reach the editor.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div/main/form/div/input').nth(0)
        ...
        await expect(frame.locator('text=WPM').first).to_be_visible(timeout=3000)
        await expect(frame.locator('xpath=//*[contains(text(), "WPM")]').first).to_be_visible(timeout=3000)
